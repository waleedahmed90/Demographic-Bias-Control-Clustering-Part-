...
# #threshold usage =5
path_gender = './Usage_Trend_15Min/Gender_Trending.gz'
path_race = './Usage_Trend_15Min/Race_Trending.gz'
path_age = './Usage_Trend_15Min/Age_Trending.gz'
...
with gzip.open('./Hashtags_Tweets_Counts/Sorted_Dictionary.gz', 'rt') as T4:
...
try:
	os.mkdir('./Clusters_TrendUsage_Demographics')
except:
	shutil.rmtree('./Clusters_TrendUsage_Demographics', ignore_errors=True)
	os.mkdir('./Clusters_TrendUsage_Demographics')


with gzip.open('./Clusters_TrendUsage_Demographics/Gender_Trend_Clusters.gz', 'wb') as f1:
	f1.write(json.dumps(Gender_Clusters).encode('utf-8'))
f1.close()
print("<./Clusters_TrendUsage_Demographics/Gender_Trend_Clusters.gz>:::::Written")
	
with gzip.open('./Clusters_TrendUsage_Demographics/Race_Trend_Clusters.gz', 'wb') as f2:
	f2.write(json.dumps(Race_Clusters).encode('utf-8'))
f2.close()
print("<./Clusters_TrendUsage_Demographics/Race_Trend_Clusters.gz>:::::Written")

with gzip.open('./Clusters_TrendUsage_Demographics/Age_Trend_Clusters.gz', 'wb') as f3:
	f3.write(json.dumps(Age_Clusters).encode('utf-8'))
f3.close()
print("<./Clusters_TrendUsage_Demographics/Age_Trend_Clusters.gz>:::::Written")
